            ...
            >
              ...
              <form onSubmit={handleSubmit(onSubmit)} className={styles.modalForm}>
                ...
                <div className={styles.row3}>
                  <div className={`${styles.field} ${temVariacao ? styles.disabledField : ''}`}>
                    <label>Preço (R$) {temVariacao ? '' : '*'}</label>
                    <input
                      className="input-field"
                      type="number"
                      step="0.01"
                      min="0"
                      disabled={temVariacao}
                      {...register('preco', { required: !temVariacao ? 'Preço obrigatório' : false })}
                    />
                    {errors.preco && <span className={styles.fieldErr}>{errors.preco.message}</span>}
                    {temVariacao && <span className={styles.infoText}>Definido nas variações</span>}
                  </div>
                  <div className={`${styles.field} ${temVariacao ? styles.disabledField : ''}`}>
                    <label>Estoque {temVariacao ? '' : '*'}</label>
                    <input
                      className="input-field"
                      type="number"
                      min="0"
                      disabled={temVariacao}
                      {...register('estoque', { required: !temVariacao ? 'Estoque obrigatório' : false })}
                    />
                    {errors.estoque && <span className={styles.fieldErr}>{errors.estoque.message}</span>}
                    {temVariacao && <span className={styles.infoText}>Soma das variações</span>}
                  </div>
                  <div className={styles.field}>
                    <label>Status</label>
